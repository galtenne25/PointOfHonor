...
import { Plus, ChevronLeft, ChevronRight, Map as MapIcon } from 'lucide-react'
...
import { ListSkeleton, EmptyState, ErrorState, Button } from '../components/ui'
  ...
  return (
    <div className="flex flex-col gap-4 pb-6" dir="rtl">
      ...
      {routesLoading ? (
        ...
        <ErrorState title="שגיאה בטעינת המסלולים" message={routesError} />
      ) : filteredRoutes.length === 0 ? (
        <EmptyState
          icon={MapIcon}
          title="לא נמצאו מסלולים"
          message="נסה לשנות את הסינון או את מילות החיפוש"
          action={!noFiltersActive && (
            <Button size="sm" rounded="full" onClick={resetRouteFilters}>
              נקה סינון
            </Button>
          )}
        />
